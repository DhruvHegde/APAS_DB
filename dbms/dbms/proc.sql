...
-- =================================================================
-- PROCEDURE 5: sp_applyNormalization
-- Normalize the weighted scores of the employees
-- =================================================================
DELIMITER //
CREATE PROCEDURE `sp_applyNormalization`(
    IN p_cycle_id INT,
    IN p_admin_id INT,
    IN p_top_percent DECIMAL(5,2),
    IN p_bottom_percent DECIMAL(5,2)
)
MODIFIES SQL DATA
BEGIN
    SET @app_user_id = p_admin_id;
    ...
    WITH RankedEmployees AS (
        SELECT 
            ...
            `cycle_id`,
            -- 
            -- FIX 1: Use CUME_DIST() * 100 to get a true percentile (1-100).
            -- NTILE() was incorrect for this use case.
            --
            (CUME_DIST() OVER (ORDER BY `weighted_score` ASC) * 100) as percentile_rank
        FROM 
            `final_ratings`
        WHERE 
            `cycle_id` = p_cycle_id
    ...
    UPDATE `final_ratings` fr
    JOIN RankedEmployees re ON fr.employee_id = re.employee_id AND fr.cycle_id = re.cycle_id
    SET 
        fr.normalized_score = re.percentile_rank,
        
        fr.final_rank = CASE 
            --
            -- FIX 2: Use > for the top percentile to get the correct distribution.
            -- e.g., for Top 20%, we want ranks 81-100, so > 80.
            --
            WHEN re.percentile_rank > (100.00 - p_top_percent) THEN 'Top Performer'
            
            WHEN re.percentile_rank <= p_bottom_percent THEN 'Low Performer'
