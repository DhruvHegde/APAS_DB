...
-- table for organisational goals
CREATE TABLE `company_goals` (
    `company_goal_id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `cycle_id` INT UNSIGNED NOT NULL,
    `goal_title` VARCHAR(255) NOT NULL,
    `goal_description` TEXT,
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    
    -- Goals are tied to a cycle
    FOREIGN KEY (`cycle_id`) REFERENCES `appraisal_cycles`(`cycle_id`) ON DELETE CASCADE
);

ALTER TABLE `final_ratings`
ADD COLUMN `normalized_score` DECIMAL(5,2) NULL COMMENT 'Percentile rank (1-100)'
